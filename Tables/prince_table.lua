prince_codes = {
  ...
  function search_code(page, line, word)
    
    flag = false

    for key, value in pairs(prince_codes) do

      if page == value.Page and
         line == value.Line and
         word == value.Word
      then
          code = value.Code
  ...
  code_found = search_code(page_input, line_input, word_input )
